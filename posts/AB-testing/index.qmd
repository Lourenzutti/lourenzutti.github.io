...
- **Sample size:** Fixed -- determined before the experiment begins and remains unchanged.<br><br>
...
Suppose you want to test the difference between two <em>independent</em> samples' means. The scenarios to
be considered:
...
# It is just a function to simulate the data.
set.seed(12)

#########################################################
# Generate a Simulated Dataset of Website Subscriptions #
#########################################################
take_sample <- function(batch_size, n_batches, replicates) {
    return(
        tibble(
            replicate = rep(1:replicates, 
                            each = batch_size * n_batches),
            batch = rep(rep(1:n_batches, each = batch_size), 
                        times = replicates),
            website = as_factor(rep(c("Website A", "Website B"), 
                                    each = batch_size/2, 
                                    times = n_batches * replicates)),
            subscribed = sample(factor(c("yes", "no"), levels = c('yes', 'no')), 
                                batch_size * n_batches * replicates, 
                                replace = TRUE, p=c(0.5, 0.5))
        ) 
    )
}
##########################################
# Simulate all the data that we will use #
##########################################
sample_viewers_source <- take_sample(50, 10, 100)     

sample_viewers_source_pvalue <- 
  map_dfr(1:10, function(b) {
    sample_viewers_source %>%
      filter(batch <= b) %>%  # Include all batches up to current batch number
      group_by(replicate) %>%
      nest() %>%
      mutate(
        cumulative_batch = b,
        pvalue = map_dbl(data, function(df) {
          prop_test(df, subscribed ~ website, 
                   order = c("Website B", "Website A")) %>% 
            pull(p_value)
        })
      )
  }) %>% 
  rename(batch = cumulative_batch)
    
################################################
# Function to collect extra batches of viewers #
################################################
collect_new_batch_of_sample_viewer <- function(sample_viewers){
  rep <- sample_viewers$replicate[1] 
  b <- max(sample_viewers$batch)
  return(
      sample_viewers_source %>% filter(replicate == rep, batch <= b + 1)
  ) 
}

##########################################
# Get the first batch of first replicate #
##########################################
sample_viewers <- 
    sample_viewers_source %>% 
    filter(batch == 1 & replicate == 24)
...
# Note that we now have the more rows but the first 50 rows are the same 
# as in the first batch.

sample_viewers <- collect_new_batch_of_sample_viewer(sample_viewers)

sample_viewers %>% print(n = 100) 
```
...
To investigate the rate, we need to conduct this experiment multiple times and see if the rate of false positives is close to 5% (our specified significance level). Let's do this experiment 100 times.
...
As you can see, following the prescribed methodology above would yield a false positive rate of 15%, roughly four times the nominal rate of 5%. The sequential hypothesis tests significantly inflate the type I error. 
