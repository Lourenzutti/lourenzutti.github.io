...
- Neuron in Layer 3:
$$x^{(3)}_1 = \max\left\{0, z^{(3)}_1\right\} = 0.0017178 \textcolor{white}{\sum_{i=1}^{2}}$$
...
$$ {#eq-matrix-receptor}
where $\mathbf{1}$ is a column vector of ones with $n$ rows and $T$ stands for transpose. The operation $\mathbf{1}\left(\mathbf{b}^{(l)}\right)^T$ is Python's broadcast.
