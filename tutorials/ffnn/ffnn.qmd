...
- **Receptors** (for the $i$th training sample): $$z^{(l)}_{i, k} =\sum_{j=1}^{n^{(l-1)}} w^{(l)}_{j, k}x^{(l-1)}_{i,j} + b^{(l)}_k,\quad l=1,...,L, \quad \text{and} \quad j=1,...,n^{(l)}$$
...
<img src="images/nn8.svg" width="60%">
...
$$z^{(2)}_1 =\sum_{i=1}^{3} w^{(2)}_{i, 1}x^{(1)}_i = -0.0304$$
$$z^{(2)}_2 =\sum_{i=1}^{3} w^{(2)}_{i, 2}x^{(1)}_i =  0.0245$$

- Receptor in Layer 3:
$$z^{(3)}_1 =\sum_{i=1}^{2} w^{(3)}_{i, 1}x^{(1)}_i = 0.0108$$
...
- Neurons in Layer 1:
$$x^{(1)}_1 = \frac{1}{1+e^{z^{(1)}_1}} = 0.4221$$
$$x^{(1)}_2 = \frac{1}{1+e^{z^{(1)}_2}} = 0.6938$$
$$x^{(1)}_3 = \frac{1}{1+e^{z^{(1)}_3}} = 0.3972$$

<br>

- Neurons in Layer 2:
$$x^{(2)}_1 = \frac{1}{1+e^{z^{(2)}_1}} = 0.4923$$
$$x^{(2)}_2 = \frac{1}{1+e^{z^{(2)}_2}} = 0.5061$$

<br>

- Neuron in Layer 3:
$$x^{(3)}_1 = \frac{1}{1+e^{z^{(3)}_1}} = 0.5027$$
...
- Receptors:
$$
{\bf{Z}}^{(l)} = {\mathbf{X}^{(l-1)}} \mathbf{W}^{(l)} + \mathbf{b}^{(l)}
$$ {#eq-matrix-receptor}
...
<img src="images/nn10.svg" width="100%">
...
W3 = np.round(rng.uniform(-0.1, 0.1, size=(2,1)), 2)
