...
- Receptors: $$z^{(l)}_j =\sum_{i=1}^{n^{(l-1)}} w^{(l-1)}_{i, j}x^{(l-1)}_i,\quad l=1,...,L, \quad \text{and} \quad j=1,...,n^{(l)}$$
- Neurons: $$x^{(l)}_j=a\left(z^{(l)}_j\right),\quad l=1,...,L, \quad \text{and} \quad j=1,...,n^{(l)}$$
...
In order for us to go through an example of the feed-forward part of the neural network, let us get some synthetic data with two features as well as define some values for the weights.     
